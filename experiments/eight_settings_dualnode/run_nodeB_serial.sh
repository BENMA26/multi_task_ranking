#!/bin/bash
#SBATCH --job-name=mtl8_nodeB_p2
#SBATCH --output=/work/home/maben/project/rec_sys/projects/multi_task_ranking/experiments/eight_settings_dualnode/slurm/nodeB_%j.out
#SBATCH --error=/work/home/maben/project/rec_sys/projects/multi_task_ranking/experiments/eight_settings_dualnode/slurm/nodeB_%j.err
#SBATCH --ntasks=1
...
RUN_ID=${RUN_ID:-nodeB_p2_$(date +%Y%m%d_%H%M%S)}
...
run_case() {
    ...
    for raw_seed in "${SEED_LIST[@]}"; do
        ...
        echo "=========================================="
        echo "[NODE-B] setting=${setting} seed=${seed}"
        echo "=========================================="
...
echo "[DONE] nodeB serial run finished. ok=${ok_count} fail=${fail_count}"

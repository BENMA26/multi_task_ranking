#!/bin/bash
#SBATCH --job-name=mtl8_nodeC_p3
#SBATCH --output=/work/home/maben/project/rec_sys/projects/multi_task_ranking/experiments/eight_settings_dualnode/slurm/nodeC_%j.out
#SBATCH --error=/work/home/maben/project/rec_sys/projects/multi_task_ranking/experiments/eight_settings_dualnode/slurm/nodeC_%j.err
#SBATCH --ntasks=1
...
RUN_ID=${RUN_ID:-nodeC_p3_$(date +%Y%m%d_%H%M%S)}
...
run_case() {
    ...
    for raw_seed in "${SEED_LIST[@]}"; do
        ...
        echo "=========================================="
        echo "[NODE-C] setting=${setting} seed=${seed}"
        echo "=========================================="
...
echo "[DONE] nodeC serial run finished. ok=${ok_count} fail=${fail_count}"

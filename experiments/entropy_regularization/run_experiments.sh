#!/bin/bash
#SBATCH --job-name=mtl_entropy
#SBATCH --output=logs/mtl_entropy_%j.out
#SBATCH --error=logs/mtl_entropy_%j.err
#SBATCH --ntasks=1
...
# ── 实验矩阵 ──────────────────────────────────────────────────────────────
# Entropy 正则化实验（解决 MMOE/MOE 门控极化问题）
# 模型：mmoe（MMOE 有独立门控，更容易出现极化问题）
# 实验组合：
#   1. baseline: 无 entropy, 无 upgrad
#   2. entropy_only: 有 entropy, 无 upgrad
#   3. upgrad_only: 无 entropy, 有 upgrad
#   4. entropy_upgrad: 有 entropy, 有 upgrad
# 共 4 组实验
...
for MODEL in "${MODELS[@]}"; do
    for i in "${!USE_ENTROPY_REGS[@]}"; do
        ...
        if [ "$USE_ENTROPY" == "true" ] && [ "$USE_TORCHJD" == "true" ]; then
            EXP_NAME="${MODEL}_esmm_entropy_upgrad"
            ...
            EXP_NAME="${MODEL}_esmm_entropy_only"
            ENTROPY_FLAGS="--use_entropy_reg --lambda_entropy 0.01"
            TORCHJD_FLAGS=""
        elif [ "$USE_TORCHJD" == "true" ]; then
            EXP_NAME="${MODEL}_esmm_upgrad_only"
            ENTROPY_FLAGS=""
            TORCHJD_FLAGS="--use_torchjd --aggregation_method upgrad"
        else
        ...
        echo "Model: ${MODEL} | Entropy: ${USE_ENTROPY} | UPGrad: ${USE_TORCHJD}"
        echo "=========================================="

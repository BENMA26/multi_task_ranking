#!/usr/bin/env python3
"""
Aggregate project-specific HP-search results across seeds and output:
1) grouped metrics per setting
2) best setting per (stage, model)
"""
...
def infer_stage_model(setting: str) -> tuple[str, str]:
    stage = "other"
    model = "other"
    if setting.startswith("projhp_normal_"):
        stage = "normal"
    elif setting.startswith("projhp_entropy_"):
        stage = "entropy"
    elif setting.startswith("projhp_jd_"):
        stage = "jd"
    elif setting.startswith("projhp_asym_"):
        stage = "asym"

    if "_share_bottom_" in setting:
        model = "share_bottom"
    elif "_mmoe_" in setting:
        model = "mmoe"
    elif "_ple_" in setting:
        model = "ple"
    return stage, model


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Aggregate project HP search results")
    p.add_argument(
        "--input_csv",
        type=Path,
        default=Path("experiments/final_ali_ccp/outputs/project_compare/hp/hp_search_summary.csv"),
    )
    p.add_argument(
        "--out_csv",
        type=Path,
        default=Path("experiments/final_ali_ccp/outputs/project_compare/hp/hp_search_grouped.csv"),
    )
    p.add_argument(
        "--out_best_csv",
        type=Path,
        default=Path("experiments/final_ali_ccp/outputs/project_compare/hp/hp_best_by_stage_model.csv"),
    )
    return p.parse_args()
...
def main() -> None:
    ...
    with args.input_csv.open("r", encoding="utf-8") as f:
        for row in csv.DictReader(f):
            ...
            stage, model = infer_stage_model(base)
            g = groups.setdefault(
                base,
                {
                    "setting": base,
    ...
    out_rows.sort(key=lambda r: (r["stage"], r["model"], -mean_for_sort(r)))
    ...
    for row in best_rows:
        print(
            "[BEST] "
            f"stage={row['stage']} model={row['model']} setting={row['setting']} "

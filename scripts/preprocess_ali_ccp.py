...
np.random.seed(2022)
'''
common_features 文件 - 用户级别的特征 
sample_skeleton 文件 - 每条曝光样本的特征

原始CSV文件
    ↓
preprocess_data(train)   ← 解析特征、构建词表、编码
preprocess_data(test)    ← 用训练集词表对测试集编码
    ↓
reduce_mem()             ← 压缩内存占用
    ↓
MinMaxScaler             ← 归一化dense特征
    ↓
保存 train / val / test CSV

稀疏特征（sparse_columns）：类别型特征，如用户ID、商品类目等，共24个字段，最终会被编码为整数索引（用于Embedding）
稠密特征（dense_columns）：数值型特征，共8个字段，加上前缀 D 区分，最终会被归一化到 [0, 1]
